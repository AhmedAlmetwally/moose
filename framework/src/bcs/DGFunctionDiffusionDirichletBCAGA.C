...
#include "DGFunctionDiffusionDirichletBCAGA.h"
...
registerMooseObject("MooseApp", DGFunctionDiffusionDirichletBCAGA);

// defineLegacyParams(DGFunctionDiffusionDirichletBCAGA);

InputParameters
DGFunctionDiffusionDirichletBCAGA::validParams()
{
...
DGFunctionDiffusionDirichletBCAGA::DGFunctionDiffusionDirichletBCAGA(const InputParameters & parameters)
...
Real
DGFunctionDiffusionDirichletBCAGA::computeQpResidual()
{
...
Real
DGFunctionDiffusionDirichletBCAGA::computeQpJacobian()
{
  const unsigned int elem_b_order = _var.order();
  double h_elem =
      _current_elem_volume / _current_side_volume * 1. / Utility::pow<2>(elem_b_order);
