...
#include "DGCGFunctionDiffusionDirichletBCAGA.h"
...
registerMooseObject("MooseApp", DGCGFunctionDiffusionDirichletBCAGA);

// defineLegacyParams(DGCGFunctionDiffusionDirichletBCAGA);

InputParameters
DGCGFunctionDiffusionDirichletBCAGA::validParams()
{
...
DGCGFunctionDiffusionDirichletBCAGA::DGCGFunctionDiffusionDirichletBCAGA(const InputParameters & parameters)
...
Real
DGCGFunctionDiffusionDirichletBCAGA::computeQpResidual()
{
  ...
  r += _sigma[_qp] * _diff[_qp] / h_elem * (_u[_qp] - fn) * _test[_i][_qp];

  return r;
}

Real
DGCGFunctionDiffusionDirichletBCAGA::computeQpJacobian()
{
  const unsigned int elem_b_order = _var.order();
  double h_elem =
      _current_elem_volume / _current_side_volume * 1. / Utility::pow<2>(elem_b_order);
