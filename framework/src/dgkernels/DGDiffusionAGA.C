...
registerMooseObject("MooseApp", DGDiffusionAGA);

//defineLegacyParams(DGDiffusionAGA);

InputParameters
DGDiffusionAGA::validParams()
{
...
DGDiffusionAGA::DGDiffusionAGA(const InputParameters & parameters)
  : DGKernel(parameters),
...
Real
DGDiffusionAGA::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;
  ...
  switch (type)
  {
    case Moose::Element:
      ...
      r += _epsilon * 0.5 * (_u[_qp] - _u_neighbor[_qp]) * _diff[_qp] * _grad_test[_i][_qp] *
           _normals[_qp];
      r += _sigma[_qp] * _diff[_qp] / h_elem * (_u[_qp] - _u_neighbor[_qp]) * _test[_i][_qp];
      break;
    ...
    case Moose::Neighbor:
      ...
      r -= _sigma_neighbor[_qp] * _diff_neighbor[_qp] / h_elem * (_u[_qp] - _u_neighbor[_qp]) * _test_neighbor[_i][_qp];
      break;
  }
...
Real
DGDiffusionAGA::computeQpJacobian(Moose::DGJacobianType type)
{
  Real r = 0;
