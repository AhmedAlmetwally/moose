#include "EGCGFunctionDiffusionDirichletBCAGA.h"
...
registerMooseObject("MooseApp", EGCGFunctionDiffusionDirichletBCAGA);

InputParameters
EGCGFunctionDiffusionDirichletBCAGA::validParams()
{
...
EGCGFunctionDiffusionDirichletBCAGA::EGCGFunctionDiffusionDirichletBCAGA(const InputParameters & parameters)
...
Real
EGCGFunctionDiffusionDirichletBCAGA::computeQpResidual()
{
...
Real EGCGFunctionDiffusionDirichletBCAGA::computeQpJacobian(){ return 0; }
Real
EGCGFunctionDiffusionDirichletBCAGA::computeQpOffDiagJacobian(unsigned int jvar)
{
  const unsigned int elem_b_order = _var.order();
  double h_elem =
      _current_elem_volume / _current_side_volume * 1. / Utility::pow<2>(elem_b_order);
