...
registerMooseObject("MooseApp", EGDiffusionAGA);

InputParameters
EGDiffusionAGA::validParams()
{
...
EGDiffusionAGA::EGDiffusionAGA(const InputParameters & parameters)
  : DGKernel(parameters),
...
Real
EGDiffusionAGA::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;
...
Real EGDiffusionAGA::computeQpJacobian(Moose::DGJacobianType /*type*/) { return 0; }
Real
EGDiffusionAGA::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
  if (jvar != _v_id)
    return 0;
